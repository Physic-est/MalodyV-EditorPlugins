    ...
    if SelectedNote ~= nil and SelectedPoint ~= nil then
        ...
        for i = 1, PointCount do
            if SelectedPoint == PointType.ControlPoint + i then
                ...
                -- 補助線1
                local b1 = 100 * (ControlPoints[1].X - P1.X) / 256
                local h1m = Editor:AddSprite('h1m', 'editor-bezier-line.png')
                h1m.Alpha = 0
                h1m.HeightBeat = PointToBeat(ControlPoints[1].Y - P1.Y)

                local auxiliaryLine1 = Editor:AddSprite('m-AuxiliaryLine-1', 'editor-bezier-line.png')
                auxiliaryLine1.Beat = PointToBeat(P1.Y + ((ControlPoints[1].Y - P1.Y) / 2))
                auxiliaryLine1.X = math.floor(100 * (P1.X + ((ControlPoints[1].X - P1.X) / 2)) / 256)
                ...
                -- 補助線2
                local b2 = 100 * (P2.X - ControlPoints[1].X) / 256
                local h2m = Editor:AddSprite('h2m', 'editor-bezier-line.png')
                h2m.Alpha = 0
                h2m.HeightBeat = PointToBeat(P2.Y - ControlPoints[1].Y)

                local auxiliaryLine2 = Editor:AddSprite('m-AuxiliaryLine-2', 'editor-bezier-line.png')
                auxiliaryLine2.Beat = PointToBeat(ControlPoints[1].Y + ((P2.Y - ControlPoints[1].Y) / 2))
                auxiliaryLine2.X = math.floor(100 * (ControlPoints[1].X + ((P2.X - ControlPoints[1].X) / 2)) / 256)
